...
const Post = ({
    ...
    if (post) {
      ...
      postApi
        ...
        .then(result => {
          ...
          setStared(post.stared);
          setCreatedAt(
            dateFormat(new Date(post.createdAt), "yyyy-mm-dd HH:MM")
          );
        })
        .catch(result => {
          console.log(result);
  ...
  const goBack = () =>
    history.push(history.location.pathname.replace("post/" + postid, ""));
        ...
        <WhiteBox>
          ...
          <Content watchComment={watchComment ? 1 : 0}>
            <DateWrapper>
              ...
              <ColumnWrapper>
                <AuthorUsername to={"/user/" + authorUsername + "/posts/"}>
                  {authorUsername}
                </AuthorUsername>
